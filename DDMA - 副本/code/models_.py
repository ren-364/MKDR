...
class MKDR(nn.Module):
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        prior_med_id = torch.nonzero(torch.mm(torch.tensor(cur_diag).to(self.device), self.diag2med) > 0.5)[:,-1]
        ...
        # 获取药物HIMI编码
        drug_memory = medemdding * (1 - self.inter) + global_embeddings * self.inter
