...
from util import bigru_attention
...
class OntologyEmbedding(nn.Module):
    def __init__(self, voc, build_tree_func, med_adj=None, ddi_adj=None,
        ...
        self.med_adj = med_adj
        if self.med_adj != None:
            id_voc = build_id_voc(voc, graph_voc)
            stage_three_edges = build_stage_three_edges_(id_voc, med_adj)
            self.edges3 = torch.tensor(stage_three_edges)
        if self.ddi_adj != None:
            id_voc = build_id_voc(voc, graph_voc)
            stage_three_edges = build_stage_three_edges(id_voc, ddi_adj)
            self.edges3 = torch.tensor(stage_three_edges)
    ...
    def forward(self):
        ...
        emb = self.g(self.g(emb, self.edges1.to(emb.device)),
                     self.edges2.to(emb.device))#相当于执行了两次GAT，每个stage执行一次
        if self.ddi_adj != None or self.med_adj != None:
            emb = self.g(emb, self.edges3.to(emb.device))
