...
class DDMA(nn.Module):
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        if len(input) > 1:
            ...
            weighted2_1 = self.get_weight(weighted_values1, weighted_values_memory)
            fact2_1 = torch.mm(weighted2_1, drug_memory)  # (1, dim)
            weighted3_1 = torch.mul(weighted_values1, weighted_values_memory)
            fact3_1 = torch.mm(weighted3_1, drug_memory)  # (1, dim)
        else:
            fact2_1 = fact1_1
            fact3_1 = torch.mm(weighted_values_memory, drug_memory)  # (1, dim)


        # 输出结果
        output = self.output(torch.cat([r1, r2, priorKnowledge, fact1_1, fact2_1, fact3_1], dim=-1))  # (1, dim)
