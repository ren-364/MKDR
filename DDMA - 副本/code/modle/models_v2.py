...
# 将患者相似与其他相似相乘，患者无历史则采用当前健康状态所计算的结果
class DDMA(nn.Module):
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        if len(input) > 1:
            ...
            visit_weight1 = F.softmax(torch.mm(query1, history_keys1.t())/norm1)
            weighted_values1 = visit_weight1.mm(history_values)
            final_weight = torch.mul(weighted_values1, weighted_values_memory)
            final_memory = torch.mm(final_weight, drug_memory)
        else:
            final_weight = torch.mul(key_weights1_1, weighted_values_memory)
            final_memory = torch.mm(final_weight, drug_memory)
