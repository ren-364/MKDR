...
class DDMA(nn.Module):
    def  __init__(self, voc, vocab_size, patient_memory, ddi_adj, global_para, med2med, diag2med, ehr_adj, emd_dim=128, num_heads=4, device=torch.device('cuda:0')):
        ...
        #self.inter_probability = nn.Parameter(torch.FloatTensor(0))
        self.threshold_indices = 0.5
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        prior_med_id = torch.nonzero(torch.mm(torch.tensor(cur_diag).to(self.device), self.diag2med) >= self.threshold_indices)[:,-1]
        #prior_med_id = self.threshold_indices(prior_med_id_)[:,-1]
        ...
        key_memory = F.gumbel_softmax(topk_weight, tau=self.gumbel_tau)
