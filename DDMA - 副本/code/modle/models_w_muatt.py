...
class DDMA(nn.Module):
    def  __init__(self, voc, vocab_size, patient_memory, ddi_adj, global_para, med2med, diag2med, pro2med, emd_dim=128, num_heads=4, device=torch.device('cuda:0')):
        ...
        # MultiHeadAttention
        self.MultiHead = MultiHeadAttention(emd_dim*2, num_heads=num_heads, dropout=0.0)
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        def get_patient_memory():
            ...
            for patient in c_patient_memory:
                ...
                #o_1, diag_atten = self.MultiHead[0](i1_seq, i1_seq, i1_seq)
                #o_2, pro_atten = self.MultiHead[1](i2_seq, i2_seq, i2_seq)
                patient_representations1 = torch.cat([o1, o2], dim=-1).squeeze(dim=0)  # (seq, dim*4)
                queries1 = self.query1(patient_representations1)  # (seq, dim)
        ...
        priorKnowledge, _ = self.MultiHead(priorMed, priorMed, priorMed)
        priorKnowledge = priorKnowledge.squeeze(dim=0)
        ...
        # 输出结果
        output = self.output(torch.cat([query1, priorKnowledge, fact1_1, fact2_1, fact_memory], dim=-1))  # (1, dim)
