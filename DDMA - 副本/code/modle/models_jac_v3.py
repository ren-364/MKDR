...
class DDMA(nn.Module):
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        def jaccard(inter, union, l_cdiag, l_ndiag):
            return math.sqrt(inter**2 / min(l_cdiag, l_ndiag)*union)
        ...
        def get_patient_memory():
            ...
            for diag in b_diag_memory:
                ...
                union = len(set(diag) | set(cur_diag))
                l_ndiag = len(set(diag))
                jaccard_score = 0 if inter == 0 else jaccard(inter, union, l_cdiag, l_ndiag)
                score_diag.append(jaccard_score)
            for pro in b_pro_memory:
                inter = len(set(pro) & set(cur_pro))
                union = len(set(pro) | set(cur_pro))
                l_npro = len(set(pro))
                jaccard_score = 0 if inter == 0 else jaccard(inter, union, l_cpro, l_npro)
