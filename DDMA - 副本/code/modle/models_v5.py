...
class GetWeight(nn.Module):
    def __init__(self, r=10, l=5):
        super(GetWeight, self).__init__()
        self.r = r
        self.l = l
    def forward(self, xtensor, ytensor, memory_length):
        if memory_length > self.r:
            a = 0
        else:
            a = math.exp(-((memory_length/self.l)**2))
        result_tensor = xtensor*a + ytensor*(1-a)
...
class DDMA(nn.Module):
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        if memory_length > 0:
            ...
            final_weight = self.get_weight(weighted_values_memory, weighted_values1, memory_length)
