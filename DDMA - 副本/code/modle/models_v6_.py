...
class DDMA(nn.Module):
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        def get_patient_memory():
            ...
            _, topk_indices = torch.topk(score, self.k)

            similar_patients = [[b_diag_memory[i], b_pro_memory[i], b_med_memory[i]]for i in topk_indices]
            query_memory = []
            value_memory = torch.zeros(self.k, self.vocab_size[2]).to(self.device)
            id_memory = 0
            for patient in similar_patients:
                p_diag = mean_embedding(diagemdding[patient[0]].unsqueeze(dim=0))
                p_pro = mean_embedding(proemdding[patient[1]].unsqueeze(dim=0))
                diag_, _ = self.MultiHead[0](p_diag, p_diag, p_diag)
                pro_, _ = self.MultiHead[1](p_pro, p_pro, p_pro)
                patient_representations1 = torch.cat([diag_, pro_], dim=-1).squeeze(dim=0)  # (seq, dim*4)
                queries1 = self.query2(patient_representations1).squeeze(dim=0)  # (seq, dim)
                query_memory.append(queries1)
                value_memory[id_memory, patient[2]] = 1
        ...
        key_weights_memory = torch.mm(query1, query_memory.t())
        #topk_weight, topk_indices = torch.topk(key_weights_memory, 10)
        #key_memory = F.softmax(topk_weight, dim=-1)
        key_memory = F.softmax(key_weights_memory)
        value_memory = value_memory.squeeze(dim=0)
