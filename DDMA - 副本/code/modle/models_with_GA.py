...
class DDMA(nn.Module):
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        # 患者标准信息
        r1 = self.query2(torch.cat([o_1.squeeze(dim=0)[-1:], o1.squeeze(dim=0)[-1:]], dim=-1))
        r2 = self.query3(torch.cat([o_2.squeeze(dim=0)[-1:], o2.squeeze(dim=0)[-1:]], dim=-1))
