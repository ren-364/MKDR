...
class DDMA(nn.Module):
    def  __init__(self, voc, vocab_size, patient_memory, ddi_adj, global_para, med2med, diag2med, pro2med, emd_dim=128, num_heads=4, device=torch.device('cuda:0')):
        ...
        self.embeddings = nn.ModuleList(
            [nn.Embedding(vocab_size[i], emd_dim*2) for i in range(K)])
        self.query1 = nn.Sequential(
            nn.ReLU(),
            nn.Linear(emd_dim * 12, emd_dim * 8),  # GRU、Attention
            nn.ReLU(),
            nn.Linear(emd_dim * 8, emd_dim * 2),  # GRU、Attention
        )
        ...
        self.encoders = nn.ModuleList([nn.GRU(emd_dim * 2, emd_dim * 2, batch_first=True) for _ in range(K)])
        ...
        # MultiHeadAttention
        self.MultiHead = nn.ModuleList(
            [MultiHeadAttention(emd_dim*2, num_heads=num_heads, dropout=0.0) for _ in range(4)]
        )
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        # 获取药物HIMI编码
        drug_emd = medemdding + global_embeddings * self.inter
        ...
        def get_patient_memory():
            ...
            for diag in b_diag_memory:
                inter = set(diag) & set(cur_diag)
                union = set(diag) | set(cur_diag)
                jaccard_score = 0 if union == 0 else len(inter) / len(union)
                score_diag.append(jaccard_score)
            for pro in b_pro_memory:
                inter = set(pro) & set(cur_pro)
                union = set(pro) | set(cur_pro)
                jaccard_score = 0 if union == 0 else len(inter) / len(union)
            ...
            for patient in c_patient_memory:
                ...
                i3_seq = []
                for adm in patient:
                    src1 = mean_embedding(self.dropout(
                        self.embeddings[0](torch.LongTensor(adm[0]).unsqueeze(dim=0).to(self.device))))  # (1,1,dim)
                    src2 = mean_embedding(self.dropout(
                        self.embeddings[1](torch.LongTensor(adm[1]).unsqueeze(dim=0).to(self.device))))  # (1,1,dim)
                    src3 = mean_embedding(self.dropout(
                        self.embeddings[2](torch.LongTensor(adm[2]).unsqueeze(dim=0).to(self.device))))  # (1,1,dim)
                    i1_seq.append(src1)
                    i2_seq.append(src2)
                    if adm != input[-1]:
                        i3_seq.append(src3)
                    else:
                        i3_seq.append(torch.zeros_like(src3).to(self.device))
                ...
                o_1, diag_atten = self.MultiHead[0](i1_seq, i1_seq, i1_seq)
                o_2, pro_atten = self.MultiHead[1](i2_seq, i2_seq, i2_seq)
                o_3, med_atten = self.MultiHead[2](i3_seq, i3_seq, i3_seq)
                patient_representations1 = torch.cat([o1, o2, o3, o_1, o_2, o_3], dim=-1).squeeze(dim=0)  # (seq, dim*4)
                queries1 = self.query1(patient_representations1)  # (seq, dim)
        ...
        priorMed = self.embeddings[2](prior_med_id if len(prior_med_id) > 0 else torch.LongTensor([0]).to(self.device)).mean(dim=0, keepdim=True).unsqueeze(dim=0)
        priorKnowledge, _ = self.MultiHead[3](priorMed, priorMed, priorMed)
        priorKnowledge = priorKnowledge.squeeze(dim=0)

        #获取患者表示
        for adm in input:
            src1 = mean_embedding(self.dropout(self.embeddings[0](torch.LongTensor(adm[0]).unsqueeze(dim=0).to(self.device)))) # (1,1,dim)
            src2 = mean_embedding(self.dropout(self.embeddings[1](torch.LongTensor(adm[1]).unsqueeze(dim=0).to(self.device)))) # (1,1,dim)
            src3 = mean_embedding(self.dropout(self.embeddings[2](torch.LongTensor(adm[2]).unsqueeze(dim=0).to(self.device))))  # (1,1,dim)
            i1_seq.append(src1)
            i2_seq.append(src2)
            if adm != input[-1]:
                i3_seq.append(src3)
        ...
        key_memory = F.softmax(topk_weight, dim=-1)
        ...
        # 算当前表征匹配药物
        key_weights1_1 = F.softmax(torch.mm(query1, drug_emd.t()), dim=-1)  # (1, size) 与卷积得到的药物编码算一次权重
        fact1_1 = torch.mm(key_weights1_1, drug_emd)  # (1, dim)
        ...
        if len(input) > 1:
            ...
            fact2_1 = torch.mm(weighted_values1, drug_emd)  # (1, dim)
        else:
            fact2_1 = fact1_1

        fact_memory = torch.mm(weighted_values_memory, drug_emd)  # (1, dim)
