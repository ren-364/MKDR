...
class DDMA(nn.Module):
    def  __init__(self, voc, vocab_size, patient_memory, ddi_adj, global_para, med2med, diag2med, pro2med, emd_dim=128, num_heads=4, device=torch.device('cuda:0')):
        ...
        self.output = nn.Sequential(
            nn.ReLU(),
            nn.Linear(emd_dim * 10, emd_dim * 5),
            nn.ReLU(),
            nn.Linear(emd_dim * 5, vocab_size[2])
        )
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        if len(input) > 1:
            ...
            final_weight = weighted2_1 + weighted3_1
            fact2_1 = torch.mm(final_weight, drug_memory)  # (1, dim)
        else:
            fact2_1 = torch.mm(weighted_values_memory, drug_memory)
            #fact3_1 =   # (1, dim)
