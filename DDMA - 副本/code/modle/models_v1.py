...
class GetWeight(nn.Module):
    def __init__(self, p=0.5):
        ...
        self.p = p
    def forward(self, xtensor, ytensor):
        result_tensor = torch.maximum(xtensor, ytensor)
        mask = torch.logical_and(xtensor < self.p, ytensor < self.p)
        result_tensor[mask] = 0
        return result_tensor
# 取患者自身相似度和其他患者相似度的最大值，并对最小概率进行筛选以缓解噪声的影响
class DDMA(nn.Module):
    def  __init__(self, voc, vocab_size, patient_memory, ddi_adj, global_para, med2med, diag2med, pro2med, emd_dim=128, num_heads=4, device=torch.device('cuda:0')):
        ...
        self.output = nn.Sequential(
            nn.ReLU(),
            nn.Linear(emd_dim * 10, emd_dim * 8),
            nn.ReLU(),
        ...
        self.get_weight = GetWeight(0.3)
        self.weight_liner = nn.Sequential(
            nn.Linear(vocab_size[2] * 2, vocab_size[2]  * 1),
            nn.ReLU()
        )
    ...
    def forward(self, input, mol_data, average_projection):
        ...
        #获取层次编码
        diagEmd, proEmd, medEmd = self.ontologyEmd['diag'], self.ontologyEmd['pro'], self.ontologyEmd['med']
        diagemdding, proemdding, medemdding = diagEmd(), proEmd(), medEmd()
        ...
        def get_patient_memory():
            ...
            for patient in c_patient_memory:
                ...
                for idx, adm in enumerate(patient):
                    ...
                    i1_seq.append(src1)
                    i2_seq.append(src2)
                    gru_i1_seq.append(src1)
                    gru_i2_seq.append(src2)
                    value_memory[id_memory, adm[2]] = 1
                    id_memory = id_memory + 1

                i1_seq = torch.cat(i1_seq, dim=1)  # (1,seq,dim)
                i2_seq = torch.cat(i2_seq, dim=1)  # (1,seq,dim)
                gru_i1_seq = torch.cat(gru_i1_seq, dim=1)  # (1,seq,dim)
                gru_i2_seq = torch.cat(gru_i2_seq, dim=1)  # (1,seq,dim)
                ...
                queries1 = self.query2(patient_representations1)  # (seq, dim)
        ...
        # 获取药物HIMI编码
        drug_memory = medemdding + global_embeddings * self.inter
        ...
        if len(input) > 1:
            ...
            final_weight = torch.cat((weighted_values1, weighted_values_memory), dim=1)
            final_weight = self.weight_liner(final_weight)
            final_memory = torch.mm(final_weight, drug_memory)  # (1, dim)
        else:
            final_memory = torch.mm(weighted_values_memory, drug_memory)

        # 输出结果
        output = self.output(torch.cat([r1, r2, priorKnowledge, fact1_1, final_memory], dim=-1))  # (1, dim)
