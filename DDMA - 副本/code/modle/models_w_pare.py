    ...
    def forward(self, input, mol_data, average_projection):
        ...
        def get_patient_memory(num):
            ...
            for patient in self.patient_memory:
                if patient[0][3] != p_id:
                        choose_memory.append(patient)
            choose_memory = random.sample(choose_memory, num)
            for patient in choose_memory:
                for adm in patient:
        ...
        # 获取全局患者相似度
        query_memory, value_memory = get_patient_memory(1000)
