...
def build_id_voc(voc, graph_voc):
     id_voc = {key_A: graph_voc.word2idx[value_A] for key_A, value_A in voc.idx2word.items()}
     return id_voc



def build_stage_three_edges(id_voc, ddi_adj):
    """
    :param id_voc:
    :param ddi_adj:
    :return: edge_idx [[1,2,3],[0,1,0]]
    """
    edge_idx = []
    for [i, j] in torch.nonzero(ddi_adj):
        edge_idx.append((id_voc[int(i)], id_voc[int(j)]))
...
def build_stage_three_edges_(id_voc, adj):
    """
    :param id_voc:
    :param ddi_adj:
    :return: edge_idx [[1,2,3],[0,1,0]]
...
def expand_level2_pro():
    level2_pro = ['00', '01-05', '06-07', '08-16', '18-20', '21-29',
              '30-34','35-39', '40-41', '42-54', '55-59', '60-64',
              '65-71','72-75', '76-84', '85-86', '87-99', '17']
    level2_expand_pro = {}
    for i in level2_pro:
        tokens = i.split('-')
        if len(tokens) == 1:
            level2_expand_pro[i] = i
        else:
            for j in range(int(tokens[0]), int(tokens[1]) + 1):
                level2_expand_pro["%02d" % j] = i
    return level2_expand_pro
...
def build_icd9_pro_tree(unique_codes):
    res = []
    graph_voc = Voc()

    root_node = 'icd9_pro_root'
    level3_dict = expand_level2_pro()
    for code in unique_codes:
        level1 = code
        level2 = level1[:2]
        level3 = level3_dict[level2]
