...
def gender_group(gender):
    if gender == 'M':
        ...
        raise ValueError(f'Unkown value of gender: {gender}.')

def demo_process(patient_file, admission_file):
    patients = pd.read_csv(patient_file)
    admissions = pd.read_csv(admission_file)
    patients = patients.loc[:, ['SUBJECT_ID', 'GENDER', 'DOB']]
    patients['DOB_YEAR'] = pd.to_datetime(patients['DOB']).dt.year
    admissions = admissions.loc[:, ['SUBJECT_ID', 'HADM_ID', 'ADMITTIME', 'ETHNICITY']]
    admissions['ADM_YEAR'] = pd.to_datetime(admissions['ADMITTIME']).dt.year
    demographics = pd.merge(patients, admissions, on='SUBJECT_ID', how='inner')
    demographics['AGE'] = demographics['ADM_YEAR'] - demographics['DOB_YEAR']
    demographics['AGE'] = demographics['AGE'].apply(age_group)
    demographics['GENDER'] = demographics['GENDER'].apply(gender_group)
    demo_pd = demographics.loc[:, ['HADM_ID', 'AGE', 'GENDER']]
    return demo_pd

###### combine three tables #####


def combine_process(med_pd, diag_pd, pro_pd, demo_pd):
...
def create_patient_record(df, diag_voc, med_voc, pro_voc):
    ...
    for subject_id in df['SUBJECT_ID'].unique():
        ...
        for index, row in item_df.iterrows():
            ...
            admission.append(int(row['AGE']))
            admission.append(int(row['GENDER']))
            patient.append(admission)
        records.append(patient)
    dill.dump(obj=records, file=open('records_final_demo.pkl', 'wb'))
    return records
...
if __name__ == '__main__':
    ...
    patient_file = 'F:\projects\my\DDMA\data\\PATIENTS.csv'
    admission_file = 'F:\projects\my\DDMA\data\\ADMISSIONS.csv'

    med_structure_file = 'F:\projects\my\DDMA\data\\idx2SMILES.pkl'

    # drug code mapping files
    ndc2atc_file = 'F:\projects\my\DDMA\data\\ndc2atc_level4.csv'
    cid_atc = 'F:\projects\my\DDMA\data\\drug-atc.csv'
    ndc_rxnorm_file = 'F:\projects\my\DDMA\data\\ndc2rxnorm_mapping.txt'
    ...
    # for demographic
    demo_pd = demo_process(patient_file, admission_file)

    print('complete procedure processing')

    # combine
    data = combine_process(med_pd, diag_pd, pro_pd, demo_pd)
    statistics(data)
